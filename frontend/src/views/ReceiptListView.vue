...
const filters = reactive({
  ...
  supplier: '',
  search: '',
  /** по ID автора, если указано число (параметр может игнорироваться API) */
  created_by: '',
})

const currentPage = ref(1)

let supplierDebounceTimer = null
let searchDebounceTimer = null
let authorDebounceTimer = null

function buildParams() {
  const params = { page: currentPage.value }
  if (filters.date_from) params.receipt_date__gte = filters.date_from
  if (filters.date_to) params.receipt_date__lte = filters.date_to
  if (filters.status) params.status = filters.status
  const sup = filters.supplier.trim()
  if (sup) params.supplier_name__icontains = sup
  const q = filters.search.trim()
  if (q) params.search = q
  const auth = filters.created_by.trim()
  if (/^\d+$/.test(auth)) {
    params.created_by = Number(auth)
  }
  return params
}
...
function rowClick(id, e) {
  ...
  router.push({ name: 'receipt-detail', params: { id: String(id) } })
}

function goEdit(id, e) {
  e.stopPropagation()
  router.push({ name: 'receipt-edit', params: { id: String(id) } })
}

async function onDelete(id, e) {
  e.stopPropagation()
  if (!window.confirm('Удалить документ поступления?')) return
  try {
  ...
  <div class="sklad-list-page">
    ...
    >
      <div class="card border-0 shadow-sm rounded-3 mb-3">
        <div class="card-body py-3">
          <div class="row g-2 align-items-end flex-wrap">
            ...
            <div class="col-12 col-md-6 col-lg">
              <label class="form-label small text-muted mb-1">Автор (ID)</label>
              <input
            ...
            <div class="col-12 d-flex flex-wrap gap-2 justify-content-md-end ms-md-auto">
              ...
              <button type="button" class="btn btn-outline-secondary" disabled title="Массовое проведение не реализовано">
      ...
      <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        ...
        <i class="bi bi-arrow-down-circle display-4 text-muted" aria-hidden="true" />
        ...
        <table class="table table-hover align-middle mb-0 small">
          ...
          <tbody>
            <tr
              ...
              @keydown.enter="router.push({ name: 'receipt-detail', params: { id: String(row.id) } })"
            >
              ...
              <td>{{ truncate(row.supplier_name, 32) }}</td>
