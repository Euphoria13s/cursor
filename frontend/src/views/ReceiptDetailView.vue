...
async function load() {
  ...
  lines.value = []
  try {
    detail.value = await receiptStore.fetchOne(id.value)
    await productStore.fetchAll({ page_size: 500 })
  } catch {
    loadError.value = 'Не удалось загрузить документ поступления.'
    loading.value = false
    return
  }
  try {
    const { data } = await api.get('/receipt-lines/', {
      params: { receipt: id.value, page_size: 500 },
    })
...
async function onDelete() {
  ...
  deleteError.value = ''
  try {
    await receiptStore.remove(id.value)
        ...
        <div class="card-body p-4">
          ...
          <dl class="row mb-0 sklad-dl">
            ...
            <dd class="col-sm-8 col-lg-9 mb-3">
              {{ formatDate(detail.receipt_date) }}
            </dd>
            <dt class="col-sm-4 col-lg-3 text-muted small fw-normal">
              Поставщик
            </dt>
            <dd class="col-sm-8 col-lg-9 mb-3">
              {{ textOrDash(detail.supplier_name) }}
            </dd>
            <dt class="col-sm-4 col-lg-3 text-muted small fw-normal">
              Статус
            </dt>
