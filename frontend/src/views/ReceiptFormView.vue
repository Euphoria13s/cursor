...
async function fetchReceiptLines(receiptId) {
  const { data } = await api.get('/receipt-lines/', {
    params: { receipt: receiptId, page_size: 500 },
  })
  return data.results || data || []
}

async function deleteAllLines(receiptId) {
  const existing = await fetchReceiptLines(receiptId)
  await Promise.all(existing.map((l) => api.delete(`/receipt-lines/${l.id}/`)))
}

async function saveLines(receiptId) {
...
function validateClient() {
  ...
  if (!header.receipt_date) {
    fieldErrors.receipt_date = 'Укажите дату'
    ok = false
  }
  if (!header.supplier_name.trim()) {
    fieldErrors.supplier_name = 'Укажите поставщика'
    ok = false
  }
  if (!header.status) {
...
async function load() {
  ...
  fieldErrors._form = ''
  try {
    await receiptStore.fetchOne(id.value)
    const r = receiptStore.currentItem
    if (!r) return
    header.document_number = r.document_number || ''
    header.receipt_date = r.receipt_date || ''
    header.supplier_name = r.supplier_name || ''
    header.status = r.status || 'draft'
    header.comment = r.comment || ''

    const raw = await fetchReceiptLines(id.value)
    if (raw.length) {
...
async function onSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await receiptStore.update(id.value, head)
      await deleteAllLines(id.value)
      await saveLines(Number(id.value))
      toast.success('Поступление сохранено')
    } else {
      const created = await receiptStore.create(head)
      const rid = created?.id
      if (rid == null) throw new Error('Нет id документа')
      await saveLines(rid)
...
<template>
  <div class="sklad-form-page">
    <div class="mb-4 d-flex flex-wrap align-items-start justify-content-between gap-2">
      <div>
        ...
        <p class="text-muted small mb-0">
          {{ isEdit ? 'Редактирование' : 'Новое поступление' }}
        </p>
      </div>
    ...
    <div v-if="isEdit && receiptStore.loading && !submitting" class="text-center py-5">
        ...
        <div class="card-body p-4">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-4">
              ...
              <input
                v-model="header.receipt_date"
                type="date"
                class="form-control"
                :class="{ 'is-invalid': fieldErrors.receipt_date }"
              >
              <div v-if="fieldErrors.receipt_date" class="invalid-feedback d-block">
            ...
            <div class="col-12">
              ...
              <input
                ...
                type="text"
                class="form-control"
                :class="{ 'is-invalid': fieldErrors.supplier_name }"
              >
              <div v-if="fieldErrors.supplier_name" class="invalid-feedback d-block">
